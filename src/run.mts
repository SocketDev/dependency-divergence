...
type RunnerName = (typeof runners)[number]["name"]
type RunnerRecord<V> = [
  { cache: CacheState; includeLockfiles: boolean; name: RunnerName },
  V
]
type PackageInstallationStruct = {
  // name: string | null
  versions: RunnerRecord<string | null>[] | null
  // key off name, not name+version
  dependencies: Map<string | null, PackageInstallationStruct> | null
}
type RunnerTime = {
  elapsed_time: string
  kernel_time: string
...
type CacheState = { warmNodeModules: boolean; warmGlobalCache: boolean }
...
type RunResult = {
  ...
  cache: CacheState
  resource_usage: RunnerTime | null
  results: PackageInstallationStruct
}
...
async function* runSuite(files: Map<string, Buffer>) {
  let withLockfilesTarball: Buffer
  let withoutLockfilesTarball: Buffer
  {
    ...
    withLockfilesTarball = Buffer.from(
      await arrayBuffer(withLockfilesTarballPacker)
    )
    withoutLockfilesTarball = Buffer.from(
      await arrayBuffer(withoutLockfilesTarballPacker)
    )
  }
  ...
  for (const config of runners) {
    ...
    for (const includeLockfiles of [false, true]) {
      ...
      for (let cache of [
        ...
        let times: RunnerTime | null = null
        const results: PackageInstallationStruct = {
          // name: null,
          versions: null,
        ...
        await runnerAPI.injectManifestsAndLockfiles(
          includeLockfiles ? withLockfilesTarball : withoutLockfilesTarball
        )
        let testResult: ArrayBuffer
        try {
          ...
          yield {
            ...
            message: `${
              config.name
            } failed to install packages with config: ${JSON.stringify({
              cache,
              includeLockfiles,
            })}`,
            cause: e,
        ...
        try {
          await new Promise<void>(async (fulfill, reject) => {
        ...
        yield {
          ...
          includeLockfiles,
          cache,
          resource_usage: times as RunnerTime | null,
          results,
        } as RunResult
      }
    }
...
async function main() {
  ...
  const entries = await Promise.all(
    manifestPaths.map(async (l) => {
      const resolved = join(cwd, l)
      return [l, await fs.readFile(resolved)] as const
    })
  )
  const files = new Map<string, Buffer>(entries)
  let resource_table_headers: string[] | null = null
  const resource_table: Array<string[]> = []
  const diagnostic_table = []
  const installs: PackageInstallationStruct[] = []
  ...
  if (installs.length) {
    ...
    if (!pruned) {
      ...
      type FlattenedInstallationStruct = {
        name: string
        includeLockfiles: boolean
        warmGlobalCache: boolean
        warmNodeModules: boolean
        path: string
        version: string
      }
      function flatten(
        installs: PackageInstallationStruct,
        path: (string | null)[] = []
      ): FlattenedInstallationStruct[] {
        const rows: {
          name: string
          includeLockfiles: boolean
          warmGlobalCache: boolean
        ...
        flatten(pruned)
          ...
          .map((row, i) => {
            ...
            for (const [name, versions] of packageManagerDistinctVersions) {
              const [singleVersionForName, version] =
                matchSinglePartition(versions)
              if (singleVersionForName) {
                ...
                const distinctIncludeLockfiles = partitionBy(
                  Array.from(versions.values()).flat(1),
                  (config) => config.includeLockfiles,
                  (config) => config.version
                )
                for (const [value, versions] of distinctIncludeLockfiles) {
                  const [singleLockfilePartition, version] =
                    matchSinglePartition(versions)
                    ...
                    for (const [version, configs] of versions) {
                      ...
                      for (const config of configs) {
                        existingSummaries.push(
                          `${name} (includeLockfiles=${value}, warmGlobalCache=${config.warmGlobalCache}, warmNodeModules=${config.warmNodeModules})`
                        )
                      }
                    }
                  }
...
//#region utils
function escapeNewlines(cell: string): string {
  return cell.replaceAll(/\r?\n/g, "<br>")
}

function merge(trees: PackageInstallationStruct[]): PackageInstallationStruct {
  if (trees.length === 0)
    throw new Error("malformed merge, missing struct to merge")
  const versions: PackageInstallationStruct["versions"] = []
  const depGroups = new Map<string | null, PackageInstallationStruct[]>()
    ...
    if (sub.dependencies) {
      ...
      if (subDeps) {
        for (const [k, v] of subDeps) {
          emplace(depGroups, k, {
            insert: () => [] as PackageInstallationStruct[],
          }).push(v)
        }
      }
    }
  }
  const dependencies: PackageInstallationStruct["dependencies"] = new Map()
 ...
 * @returns
 */
function prune(tree: PackageInstallationStruct) {
 ...
 */
function emplace<K, V, M extends Map<K, V>>(
  m: M,
  k: K,
  fns: {
    insert?: () => V
...
function matchSinglePartition<A, B>(
  partitioned: Map<A, B[]>
): [true, A, B[]] | [false, null, B[]] {
