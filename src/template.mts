...
const rootImg = async () => {
  return ($rootImg ??= await (async () => {
    const imgResult = child_process.spawn(
      "docker",
      ["build", "--quiet", fileURLToPath(new URL("../", import.meta.url))],
      {
        stdio: "pipe",
      }
    )
...
const committedImages: Map<string, string> = new Map()
process.on("exit", () => {
...
async function getContainerForImage(
  imgId: string,
  info: string = "run container"
) {
  const containerResult = child_process.spawn(
    "docker",
    ["run", "--rm", "--detach", imgId, "sleep", "infinity"],
    {
      stdio: "pipe",
    }
  )
...
async function getCommitedImageForInstallPkgManager(
  install_pkg_manager: string,
  ...
  const { containerId, cleanup } = await getContainerForImage(
    await rootImg(),
    `build install_pkg_manager container for ${name}`
  )
  try {
    const install_pkg_manager_pid = child_process.spawn(
      "docker",
      [
        "exec",
        "--env",
...
async function checkStatus(child: ChildProcess, name?: string): Promise<void> {
...
export async function allocRunner(
  { name, description, install_pkg_manager, install_packages }: TestConfig
) {
  const img = await getCommitedImageForInstallPkgManager(
    install_pkg_manager,
    name
  )
  ...
  const { containerId, cleanup } = await getContainerForImage(
    img,
    `build install_packages container for ${name}`
  )
  return {
    cleanup,
    async injectManifestsAndLockfiles(inputTar: Buffer) {
        try {
    ...
    async run() {
      ...
      const out = child_process.spawn(
        "docker",
        ["cp", `${containerId}:/opt/output/`, "-"],
        {
          stdio: "pipe",
        }
      )
